...
- ✅ **Bienvenue / Au revoir** — `/welcome` et `/goodbye` : éditeur interactif (salon, message **texte** ou **embed**, activer/désactiver, bouton **Tester**). **Variables** `{user}` `{username}` `{tag}` `{server}` `{membercount}`. Envoi auto à l'arrivée / au départ. Config en SQLite.
